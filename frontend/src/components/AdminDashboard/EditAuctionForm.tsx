...
export default function EditAuctionForm() {
  ...
  const updateAuction = useMutation({
    ...
    }) => api.admin.updateAuction(id!, body),
    onSuccess: () => {
    ...
    <section className="bg-white rounded-xl border border-slate-200 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        {!isEnded && (
          <button
            type="button"
            onClick={() => {
              if (!window.confirm("¿Finalizar esta subasta? No se podrán hacer más pujas.")) return;
              setStatus.mutate("ENDED");
            }}
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-lg">
        ...
        <div className="flex flex-col-reverse sm:flex-row gap-2 pt-2">
          ...
          <button
            type="submit"
            disabled={updateAuction.isPending || !title.trim()}
